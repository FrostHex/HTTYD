	...
	if rd == null:
		...
		if buffers:
			...
			for view in view_count:
				# In VR/multiview, use a per-view framebuffer to avoid layer-count mismatch assertions.
				var framebuffer := FramebufferCacheRD.get_cache_multipass([buffers.get_color_layer(view, is_msaa_on), buffers.get_depth_layer(view, is_msaa_on)], [], 1)
				if not framebuffer.is_valid():
					push_warning("SunshineCloudsGD: Failed to create framebuffer for view %d" % view)
					continue
