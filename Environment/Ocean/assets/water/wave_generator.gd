...
func init_gpu(num_cascades : int) -> void:
	...
	var spectrum_compute_shader := context.load_shader('res://Environment/Ocean/assets/shaders/compute/spectrum_compute.glsl')
	var fft_butterfly_shader := context.load_shader('res://Environment/Ocean/assets/shaders/compute/fft_butterfly.glsl')
	var spectrum_modulate_shader := context.load_shader('res://Environment/Ocean/assets/shaders/compute/spectrum_modulate.glsl')
	var fft_compute_shader := context.load_shader('res://Environment/Ocean/assets/shaders/compute/fft_compute.glsl')
	var transpose_shader := context.load_shader('res://Environment/Ocean/assets/shaders/compute/transpose.glsl')
	var fft_unpack_shader := context.load_shader('res://Environment/Ocean/assets/shaders/compute/fft_unpack.glsl')

	# --- DESCRIPTOR PREPARATION ---
	var dims := Vector2i(map_size, map_size)
	var num_fft_stages := int(log(map_size) / log(2))

	descriptors[&'spectrum'] = context.create_texture(dims, RenderingDevice.DATA_FORMAT_R32G32B32A32_SFLOAT, RenderingDevice.TEXTURE_USAGE_STORAGE_BIT | RenderingDevice.TEXTURE_USAGE_CAN_COPY_FROM_BIT | RenderingDevice.TEXTURE_USAGE_SAMPLING_BIT, num_cascades)
